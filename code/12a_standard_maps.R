...
standard_maps <- function(FOLDER_NAME = NULL, SUBFOLDER_NAME = NULL,
  ...
  plot(x = rep(1:4, nrow(MODEL$recommandations)), y = rep(nrow(MODEL$recommandations):1, each = 4), axes = FALSE, cex = 3,
       xlim = c(0,5), ylim = c(0,nrow(MODEL$recommandations)+1), ylab = "", xlab = "",
  ...
  for(i in loop_over){
    ...
    for(j in seq_along(MONTH)){
      ...
      r_sd[r_sd <= 0] <- 1e-10 # to avoid dealing with true 0 in the plots / its just visual

      # --- 4.2.3. MESS
      r_mess <- QUERY$MESS[[MONTH[[j]]]]*-1
      if(nlyr(r_mess) > 1){r_mess <- app(r_mess, mean, na.rm = TRUE)}
      r_mess[r_mess<0] <- 1e-10 # to avoid dealing with true 0 in the plots / its just visual
      ...
      plot(r_m > quantile(terra::values(r_m), 0.75, na.rm = TRUE), col = c("white","gray80"), legend=FALSE, main = "Observations")
      # --- 4.3.2.2. Land mask
      plot(land, col = "antiquewhite4", legend=FALSE, add = TRUE)
      box("figure", col="black", lwd = 1)
      ...
      r <- bivar_map(rasterx = r_sd, rastery = r_mess, colormatrix = bivar_pal,
                     cutx = seq(0,max(terra::values(r_sd), na.rm = TRUE), length.out = 101), cuty = 0:100)
  ...
  if(CALL$ENSEMBLE == TRUE & (length(MODEL$MODEL_LIST) > 1)){
    ...
    plot(r_m > quantile(values(r_m), 0.75, na.rm = TRUE), col = c("white","gray80"), legend=FALSE, main = "Observations")
    # --- 6.3.2.2. Land mask
    plot(land, col = "antiquewhite4", legend=FALSE, add = TRUE)
    box("figure", col="black", lwd = 1)
